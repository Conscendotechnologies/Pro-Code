...
function parseRipgrepResults(text: string, query: string, context: any): RegexSearchResult[] {
	...
	for (const line of lines) {
		...
		if (match && currentPath) {
			...
			if (
				context?.currentFileType === "lwc" &&
				(currentPath.includes("/lwc/") || currentPath.includes("\\lwc\\"))
			)
				score += 0.2
...
function rankResults(results: VectorStoreSearchResult[], query: string): VectorStoreSearchResult[] {
	...
	return results
		.map((r) => {
			...
			if (/^(export )?(function|class|const|interface|type)\s/.test(String(r.payload?.codeChunk || "").trim()))
				boost *= 1.3
...
function mergeAndDeduplicateResults(
	...
	const rankedVector = rankWithSalesforceContext(
		rankWithContext(rankSalesforceResults(rankResults(vector, query), query), contextPath),
		sfCtx,
	)
	...
	for (const m of regexMatches) {
		const fp = m.filePath
		const chunk = m.text
		const overlap = unified.some(
			(u) => u.filePath === fp && m.line >= u.startLine && m.line <= u.endLine && u.codeChunk.includes(chunk),
		)
		if (overlap) continue
...
export async function codebaseSearchTool(
	...
	if (typeof maxResultsParam !== "undefined") {
		const cleaned =
			(block.params as any).size !== undefined
	...
	try {
		...
		if (!regexText || regexText.trim().length === 0 || /^No results found/i.test(regexText)) {
			...
			if (variations.length > 0) {
				const extraPromises = variations.map((v) =>
					regexSearchFiles(cwd, baseDir, escapeRegex(v), filePattern, cline.rooIgnoreController),
				)
		...
		if ((!merged || merged.length === 0) && !sfGraphSection) {
		...
		const output = `${sfGraphSection}Query: ${query}\nResults:\n\n${jsonResult.results
			.map(
				(r) =>
		...
		searchCache.set(cacheKey, {
			ts: Date.now(),
			kind: vectorResults.length > 0 ? "index" : "fallback",
			payload,
			output,
		})
	} catch (error: any) {
