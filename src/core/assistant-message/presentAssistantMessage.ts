...
export async function presentAssistantMessage(cline: Task) {
	...
	switch (block.type) {
		...
		case "tool_use":
			...
			if (STREAMED_TOOL_DEBUG_ENABLED) {
				logStreamedToolDebug("ToolUseDispatchDebug", "tool:dispatch", {
					tool: block.name,
	...
	if (shouldAdvanceStreamingContentIndex(block.partial, cline.didRejectTool, cline.didAlreadyUseTool)) {
...
export function shouldAdvanceStreamingContentIndex(
	blockPartial: boolean,
	didRejectTool: boolean,
	didAlreadyUseTool: boolean,
) {
	if (!blockPartial || didRejectTool) {
		return true
	}

	// Do not advance past a partial block just because an earlier tool in the
	// same message already completed. The same block still needs a chance to
	// finish streaming so it can be executed or explicitly skipped once complete.
